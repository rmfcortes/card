...
export class TabsPage implements OnInit {
  ...
  ngOnInit() {
    this.infoSub = this.commonService.perfil.subscribe(resp => {
      if (resp) {
        this.perfil = resp
        this.setColores()
      }
  ...
  setColores(): void {
    ...
    for (const i of iconos) {
      i.style.setProperty('--color', this.perfil.colores.iconosTabs)
      i.style.setProperty('--color-selected', this.perfil.colores.iconosTabsFocused)
      i.style.setProperty('--color-focused', this.perfil.colores.iconosTabsFocused)
    }
  }
